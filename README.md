...
Now create two text files called users.txt and pass.txt. For example type nano users.txt, then use chat.gpt to generate random usernames and passwords and paste it into the file. Press control + x, then press y, and then enter. Then check to see if your files have the usernames or passwords by doing cat users.txt or pass.txt
...
Now go to Ubuntu start-up Splunk and use this query if you want to see how many attempts were made and the location they came from(make sure that you include your index name because my index name might be different from yours):
